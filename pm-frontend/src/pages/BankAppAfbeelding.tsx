...
const BanlAppAfbeelding: React.FC = () => {
  ...
  const [laatsteVerwerkteBetalingDatum, setLaatsteVerwerkteBetalingDatum] = useState<string | undefined>(undefined);
  const [saldoOpLaatsteBetalingDatum, setSaldoOpLaatsteBetalingDatum] = useState<number | undefined>(undefined);
  ...
  useEffect(() => {
    const haalLaatsteBetaalDatumMetSaldoOp = async () => {
      if (actieveHulpvrager && ocrBankRekening) {
        setIsLoading(true);
        const id = actieveHulpvrager.id
        let token = '';
        try { token = await getIDToken() }
        ...
        const response = await fetch(`/api/v1/betalingen/hulpvrager/${id}/betalingocrvalidatie`, {
          ...
          body: JSON.stringify({
            'saldoOpLaatsteBetalingDatum': { 'rekeningNaam': ocrBankRekening?.naam } as Saldo
          })
        });
        setIsLoading(false);
        if (response.ok) {
          const result = await response.json();
          setLaatsteVerwerkteBetalingDatum(result.laatsteBetalingDatum);
          setSaldoOpLaatsteBetalingDatum(result.saldoOpLaatsteBetalingDatum.bedrag);
        } else {
          console.error("Failed to fetch data", response.status);
          setSnackbarMessage({
            message: `De configuratie voor ${actieveHulpvrager.bijnaam} is niet correct.`,
    ...
    haalLaatsteBetaalDatumMetSaldoOp();

  }, [ocrBankRekening, actieveHulpvrager, getIDToken]);
    ...
    <Box>
      ...
      <Grid container flexDirection='row' justifyContent="space-between" alignItems="center">
        ...
        <Button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Verwerken...' : ocrData ? 'Selecteer andere afbeelding' : 'Selecteer afbeelding'}
          <input
            type="file"
            accept="image/*"
      ...
      <Grid container flexDirection='row' justifyContent="flex-end">
        <Typography variant="body2" sx={{ mt: 1 }}>
          {confidence && toonUpdatedAfbeelding ? `OCR vertrouwen: ${confidence.toFixed(2)}%` : ''}
          {confidence && toonUpdatedAfbeelding && ocrBankRekening? ', ' : ''}
        </Typography>
        <br/>
        <Typography variant="body2" sx={{ mt: 1 }}>
          {ocrBankRekening ? `Bank: ${ocrBankRekening.naam}` : ''}
          {laatsteVerwerkteBetalingDatum ? `, laatste betaling: ${dayjs(laatsteVerwerkteBetalingDatum).format('D MMMM')}` : ''}
          {laatsteVerwerkteBetalingDatum && saldoOpLaatsteBetalingDatum ? ` met saldo: €${saldoOpLaatsteBetalingDatum.toFixed(2)}` : ''}
        </Typography>
